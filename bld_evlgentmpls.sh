...
#
NAME=evlgentmpls
PROJECT_DESC="Generate evlog template"
...
function usage {
	echo 
	echo "Usage:"
	echo "      bld_evlgentmpls.sh [-a <architecture] [-v <version>]"
	echo
	exit 0
}
...
cp -rf $BUILDRPM_START_DIR/user/cmd/evlgentmpls ${NAME}-${VERSION}/user/cmd
fi

cp ${NAME}-${VERSION}/user/cmd/evlgentmpls/${NAME}.spec ${HOME}/srctmp
#
# tar it up
#
