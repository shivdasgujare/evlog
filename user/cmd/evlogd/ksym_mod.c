...

{
	...
	if ( symbol[0] == '#' )
	{
		...
		{
			TRACE("Cannot allocate Module array.\n");
			return(0);
		}
		...
		{
			TRACE("Error reading module descriptor.\n");
			return(0);
		}
		close(memfd);
		...
		strcpy(mp->name, &symbol[1]);
                ...
                if ( query_module(sym_array_modules[num_modules].name, QM_INFO, \
				  &sym_array_modules[num_modules].module_info, \
				  sizeof(struct module_info), &ret) )
		{
			TRACE("query_module Error [%s]\n", strerror(errno));
			return(0);
		}
#endif
